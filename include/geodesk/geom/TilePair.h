...
public:
	...
	TilePair& operator+=(const Tile& other)
	{
		if (isNull())
		{
			tile_ = static_cast<uint32_t>(other);
			return *this;
		}
		...
		for (;;)
		{
			...
			{
				tile_ = static_cast<uint32_t>(Tile::fromColumnRowZoom(
					left, top, commonZoom))
	...
	*/
	...
	TilePair zoomedOut(int lowerZoom) const
	{
		...
		uint32_t northWest = static_cast<uint32_t>(first().zoomedOut(lowerZoom));
		uint32_t southEast = static_cast<uint32_t>(second().zoomedOut(lowerZoom));
	...
	{
		clarisma::Format::unsafe(buf, "%d/%d%s/%d%s", zoom(),
			column(), (extendsEast() ? "+" : ""),
			row(), (extendsSouth() ? "+" : ""));
	}
	...
	static TilePair covering(Tile tile, uint_fast8_t twinCode)
	{
		...
		TilePair pair = TilePair(static_cast<uint32_t>(fromColumnRowZoom(
