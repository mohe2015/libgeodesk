...
{
    template<>
    struct hash<geodesk::LineSegment>
    {
        size_t operator()(const geodesk::LineSegment& seg) const
        {
            return (hash<geodesk::Coordinate>()(seg.start) * 37) ^ hash<geodesk::Coordinate>()(seg.end);
        }
    };
}
